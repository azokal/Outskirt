...
public class Partition {
	...
	public Partition(App app, Game game, String name) {
		...
		try {
			...
			for (int i = 0; i < pattern.length; i++) {
				TreatXML(patterns.get(pattern[i].getString("id")),
						pattern[i].getFloat("timing"),
						pattern[i].getFloat("angle"));
			}
			...
			// new BasicBullet(game, children[i].getFloat("angle"),
			// Game.Color.values()[children[i].getInt("life")],
